   ...
   while !queue.empty?
     ...
     if File.file?(filename)
       ...
       Dir.new(filename).each do |child|
         unless ['..', '.','.svn', '.git', 'Box2DFlashAS3', 'ChartBackgroundsLib', 'McCuneChart'].include? child
...
f.readline) rescue nil
   ...
   if dry_run
     puts "#{count} files don't have our license header."
   else
     puts "#{count} files have been changed to include the GPL license license header."
   end
 end
