...
package com.dougmccune.controls
{
	...
	/**
	 *  Specifies whether the AxisRendererExt should drop labels
	 *  as necessary to lay out correctly.
	 *
	 *  <p>Set to <code>true</code> to cause the AxisRendererExt
	 *  to drop overlapping labels when rendering.
	 *  Labels are always rendered at regular intervals.
	 *  If necessary, the AxisRendererExt will render every other label,
	 *  or every third label, but never drop two or more consecutive labels.</p>
	 *
	 *  <p>Set to <code>false</code> to cause the AxisRendererExt
	 *  to use other schemes (rotation, scaling) to lay out the labels.</p>
	 *
	 *  <p>If you do not explicitly set this style, the AxisRendererExt defaults
	...
	/**
	 *  Specifies whether the AxisRendererExt should clip (hide) labels
	 *  that extend beyond the edge of the chart.
	 *
	 *  <p>Set to <code>true</code> to cause the AxisRendererExt
	 *  to clip labels when rendering.
	 *
	 *  <p>Set to <code>false</code> to cause the AxisRendererExt
	 *  to keep labels visible even if they extend beyond the edge of the chart.</p>
	 *
	 *  <p>If you do not explicitly set this style, the AxisRendererExt defaults
	 *  to an appropriate value based on the type of axis being rendered.</p>
	 *
	 ...
	 *  @productversion Flex 3
	 */
	[Style(name="canClipLabels", type="Boolean", inherit="no")]
	 ...
	 *  the AxisRendererExt determines an optimal angle to render
     ...
     *  <ul>
     *    <li><code>"inside"</code> -
     *    Draw labels inside the data area.</li>
     *
     *    <li><code>"outside"</code> -
     *    Draw labels in the label area.</li>
     *  </ul>
     *
     *  @langversion 3.0
     *  @playerversion Flash 9
     ...
     *  @productversion Flex 3
     */
    [Style(name="labelPlacement", type="String", enumeration="inside,outside", inherit="no")]

	[ResourceBundle("charts")]
	
	/**
	 *  You use the AxisRendererExt class to describe
	 ...
	 *   
	 *  <p>AxisRendererExt objects inherit some of their visual properties
	 ...
	 *  either on the AxisRendererExt or on the enclosing chart.</p> 
	 *  
	 *  <p>When positioning and sizing labels, the AxisRendererExt
	 *  takes a minimum amount of the chart's available space.
	 *  If labels take too much space, then the AxisRendererExt scales them.
	 *  However, the AxisRendererExt does not scale the labels
	 *  to too small of a point size.
	 *  To increase readability, the AxisRendererExt chooses one
	 ...
	 *    The AxisRendererExt rotates the labels to the minimum angle
	 *    needed to prevent them from overlapping.
	 *    The maximum angle is 90 degrees.</li>
	 *  </ul>
	 *  
	 *  <p>The AxisRendererExt adjusts the boundaries of the chart's data area
	 *  to ensure that it can draw the labels without pushing outside
	 *  of the boundaries.</p>
	 *  
	 *  @mxml
	 *  
	 *  <p>The <code>&lt;mx:AxisRendererExt&gt;</code> tag inherits all the properties
	 ...
	 *  @productversion Flex 3
	 */
	public class AxisRendererExt extends DualStyleObject implements IAxisRenderer, IFocusManagerComponent
	{
//		include "../core/Version.as";
        ...
        override protected function keyDownHandler(event:KeyboardEvent):void
        {
            ...
            if (!event.shiftKey && event.ctrlKey && event.altKey && event.keyCode == Keyboard.T)
            {
                _traceEvents = !_traceEvents;
                trace(componentMessagePrefix, "_traceEvents", _traceEvents);
            }

        }

        private function get componentMessagePrefix():String
        {
            return (this.parent.parent.parent.parent.parent.parent as UIComponent).id + "..." + this.id;
        }
...
//--------------------------------------------------------------------------
		 ...
		 *  @private
		 */
		private var _otherAxes:Array /* of AxisRendererExt */; 
		 ...
		 *  @private
		 */
		protected var _labels:Array /* of ARLabelData */ = [];
		...
		/**
		 *  The base chart for this AxisRendererExt.
		 *  
		 *  @langversion 3.0
		 *  @playerversion Flash 9
		 *  @playerversion AIR 1.1
		 ...
		 *  <p>The AxisRendererExt will create one instance of this class
		 ...
		 *  how the AxisRendererExt chooses to lay out labels, 
		 ...
		 *  @productversion Flex 3
		 */
		public function set otherAxes(value:Array /* of AxisRendererExt */):void
		{
			_otherAxes = value;
		}
		 ...
		 *  <p>The AxisRendererExt will create an instance of this class
		...
		private function initStyles():Boolean
		{
			...
			var axisRendererStyle:CSSStyleDeclaration =
				HaloDefaults.createSelector("com.dougmccune.controls.AxisRendererExt", styleManager);
		...
		{
            if (_traceEvents)
                trace(componentMessagePrefix, "adjustGutters");
            ...
            if (getStyle("labelPlacement") == "inside")
                targetHeight = NaN;
			else if (adjustable.bottom == false)
...
//            trace((this.parent.parent.parent.parent.parent.parent as UIComponent).id + "..." + this.id, "---------------------- adjustGutters");
//            trace("  before", "top", gutters.top, "bottom", gutters.bottom, "left", gutters.left, "right", gutters.right);

			_gutters = workingGutters;
			
			invalidateDisplayList();

//            trace("   after", "top", gutters.top, "bottom", gutters.bottom, "left", gutters.left, "right", gutters.right);
		...
		{       
			// At this point we know the AxisRendererExt length,
		...
		private function drawLabels(showLine:Boolean):Number
		{
            if (_traceEvents)
            {
//                var firstLabel:ARLabelData = (_labels.length > 0) ? _labels[0] : null;
                trace(componentMessagePrefix, "drawLabels");
//                      "unscaledWidth", unscaledWidth, "_gutters.left", _gutters.left, "_gutters.right", _gutters.right,
//                      "unscaledHeight", unscaledHeight, "_gutters.top", _gutters.top, "_gutters.bottom", _gutters.bottom,
//                      "_labels.length", _labels.length,
//                      "firstLabel.text", firstLabel ? firstLabel.text : null,
//                      "firstLabel.instance.text", (firstLabel && firstLabel.instance is Label) ? (firstLabel.instance as Label).text : null,
//                      "firstLabel.instance.x", (firstLabel && firstLabel.instance is Label) ? firstLabel.instance.x : null
//                );
            }
			...
			var baseline:Number;
            var labelPlacementInside:Boolean = getStyle("labelPlacement") == "inside";
			...
			var canClipLabelsStyle:Object = getStyle("canClipLabels");
			var canClipLabels:Boolean = canClipLabelsStyle != false &&
				canClipLabelsStyle != "false";
			...
			if (_labelPlacement.rotation == 0)
			{
				...
				else
				{
                    if (labelPlacementInside)
                    {
                        labelY = baseline - _maxLabelHeight * scale - labelGap;
                    }
                    else
                    {
...
//                    trace("Label", thisLabel.text, ldi.instance.x);
                    thisLabel.instance.visible = !(canClipLabels && (thisLabel.instance.x < 0 || thisLabel.instance.x + thisLabel.instance.width > axisWidth));
			...
			else if (_labelPlacement.rotation > 0)
			{
				...
				else // bottom or left, positive values
				{
                    ...
                    if (labelPlacementInside)
                    {
                        // TODO: use appropriate trig calculation to make this work for all values of rotation; currently works for a vertical axis with labels rotated 90 degrees
                        labelY = baseline - labelGap;
                    }
                    else
                    {
					...
					for (i = 0; i < n; i++)
					{
                        ...
                        ldi.instance.visible = !(canClipLabels && (ldi.instance.x < 0 || ldi.instance.x + ldi.instance.height > axisWidth));
		...
		private function renderLabels():void
		{
            ...
            {
                var firstLabel:ARLabelData = (_labels.length > 0) ? _labels[0] : null;
                trace(componentMessagePrefix, "renderLabels (end)",
//                      "unscaledWidth", unscaledWidth, "_gutters.left", _gutters.left, "_gutters.right", _gutters.right,
//                      "unscaledHeight", unscaledHeight, "_gutters.top", _gutters.top, "_gutters.bottom", _gutters.bottom,
                      "_labels.length", _labels.length,
                      "firstLabel.text", firstLabel ? firstLabel.text : null,
			...
			if (showLine == true)
			{
				...
				if (_horizontal && _otherAxes)
				{
					...
					for (var i:int = 0; i < n; i++)
					{
						...
						if (otherAxis is AxisRendererExt)
						{
							otherLineStyle = AxisRendererExt(otherAxis).getStyle("axisStroke");
							otherShowLine = AxisRendererExt(otherAxis).getStyle("showLine");
						}
                ...
                var lineY:Number = baseline + (getStyle("labelPlacement") == "inside") ? 0 : (w / 2);
		...
		private function tickSize(showLine:Boolean):Number
		{
			...
			{           
				...
				case "inside":
				{
					rv = labelPlacement == "inside" ? tickLength : 0;
					break;
				}
				...
				case "outside": 
				{
					rv = labelPlacement == "inside" ? 0 : tickLength;
					break;
				}
			}
...
class ARLabelData
{
	...
	/**
	 *  @private
	 *  Modified version of value.text as part of AxisRendererExt's label function
	 */
